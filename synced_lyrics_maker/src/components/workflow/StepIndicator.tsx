...
export default function StepIndicator({
  ...
  return (
    <div className="w-full pt-6 pb-4 sm:pt-12 sm:pb-8 text-white relative select-none flex flex-col justify-center items-center">
      ...
      <div className="relative w-full max-w-4xl px-4 sm:px-8">
        ...
        {/* Track de progression (fond gris) - Hide on very small screens */}
        <div className="hidden sm:block absolute top-[2.5rem] left-[3.5rem] right-[3.5rem] h-1.5 bg-slate-800/50 rounded-full overflow-hidden backdrop-blur-sm">
        ...
        <div className="relative z-10 flex justify-between items-start">
          {STEPS_CONFIG.map((stepConfig) => {
            ...
            return (
              ...
              >
                {/* Cercle principal - Responsive sizes */}
                <motion.div
                  className={cn(
                    "w-14 h-14 sm:w-20 sm:h-20 rounded-full flex items-center justify-center text-xl sm:text-2xl relative transition-all duration-500",
                ...
                >
                  ...
                  >
                    {isLocked ? (
                       <Lock className="w-5 h-5 sm:w-6 sm:h-6 opacity-50" />
                    ) : (
                       <Icon className={cn("w-6 h-6 sm:w-8 sm:h-8", isActive && "text-white drop-shadow-md")} />
                    )}
                  </motion.div>
                ...
                {/* Labels - Compact on mobile */}
                <motion.div
                  className="mt-2 sm:mt-4 text-center flex flex-col items-center gap-0.5 sm:gap-1"
                ...
                >
                  ...
                  <span className={cn(
                    "hidden sm:inline text-xs font-medium px-2 py-0.5 rounded-full border border-white/5 bg-black/20 backdrop-blur-sm transition-colors duration-300",
